"""Split the crop-folder dataset for Ultralytics YOLO26 classification."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create stratified folders for YOLO26 image classification.")
    parser.add_argument("--source", type=Path, required=True, help="Path to raw dataset folder")
    parser.add_argument("--output", type=Path, default=Path("classification_dataset_yolo26"), help="Output directory for formatted dataset")
    ...
    try:
        ...
        raise
    print(f"Created YOLO26 classification dataset at: {output}")
