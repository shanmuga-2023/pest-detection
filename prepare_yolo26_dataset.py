"""Materialize an annotated dataset in Ultralytics YOLO26 detection format."""
...
def process_coco_dataset(source: Path, output: Path, coco_splits: dict[str, Path]):
    ...
    try:
        ...
        print(f"Successfully created YOLO26 detection dataset at: {output}")
...
def main():
    parser = argparse.ArgumentParser(description="Convert dataset (COCO format or mirrored LabelMe annotations) to YOLO26 detection format.")
    ...
    parser.add_argument("--output", type=Path, required=True, help="Output directory for YOLO26 dataset")
